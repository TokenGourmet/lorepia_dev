...
This is the evidence index for the current commit, not a roadmap checkbox list. `NOT RUN` is intentional until a qualifying record described in [`README.md`](README.md) is attached. Replace a state with `PASS`, `FAIL`, or `BLOCKED` plus a direct evidence link; never delete failure evidence when a fallback is chosen.
...
| JavaScript watchdog | NOT RUN | NOT RUN | NOT RUN | NOT RUN | NOT RUN | None |
| iframe IPC and broker denial | NOT RUN | NOT RUN | NOT RUN | NOT RUN | NOT RUN | None |
...
| Android Store-Safe decision | JS/LUA OFF | Written policy clearance and real-device defenses do not yet exist |
| iOS Store-Safe decision | JS/LUA OFF | Written policy clearance and real-device defenses do not yet exist |
