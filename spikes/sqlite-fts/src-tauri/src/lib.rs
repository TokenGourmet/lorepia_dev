...
macro_rules! generate_sqlite_handler {
    ($($command:ident),+ $(,)?) => {
...
pub fn run() {
    tauri::Builder::default()
        .invoke_handler(with_sqlite_app_commands!(generate_sqlite_handler))
        .run(tauri::generate_context!())
        .expect("failed to run LorePia SQLite/FTS5 spike");
}
...
mod command_surface_tests {
    ...
    fn native_command_surface_is_exact() {
        const COMMANDS: &[&str] = with_sqlite_app_commands!(command_names);
        assert_eq!(COMMANDS, &["run_sqlite_m1_probe"]);
    }
}
