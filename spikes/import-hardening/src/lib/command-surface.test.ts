...
describe("frontend native command surface", () => {
  it("contains one direct literal invoke and only the no-argument M-1 probe", () => {
    ...
    const invocations = allProductionSources().flatMap((sourcePath) => {
      ...
      const visit = (node: ts.Node): void => {
        if (
          ...
          unsafeInvokeBindingUses.push({ source: sourceName, use: node.parent.getText(syntax) });
        }
    ...
    expect(coreImports).toEqual([
      { source: "import-probe.ts", imported: "invoke", local: "invoke" },
    ]);
    expect(unsafeInvokeBindingUses).toEqual([]);
    expect(invocations).toEqual([
      {
        callee: "invoke",
        command: "run_import_hardening_m1_probe",
        arguments: 1,
      },
    ]);
  });

  it("imports invoke in only the bounded protocol module", () => {
    ...
    expect(importers).toEqual(["import-probe.ts"]);
  });

  it("never reflects raw caught failures or bypasses the imported API", () => {
    const production = allProductionSources()
      .map((sourcePath) => readFileSync(sourcePath, "utf8"))
      .join("\n");
    expect(production).not.toMatch(/JSON\.stringify\(\s*(?:error|rawFailure)\s*\)/);
  ...
  it("keeps the probe page plain and limited to one button and one receipt", () => {
    ...
    expect(page).not.toMatch(/<style\b|\bclass=|\btransition:|\banimate:|\buse:/);
  });
});
...
describe("native capability surface", () => {
  it("grants the exact probe command only to the main WebView", () => {
    ...
    expect(capability).toEqual({
      ...
      identifier: "default",
      description: "Exact import-hardening M-1 probe capability for the trusted main WebView",
      webviews: ["main"],
      permissions: ["allow-run-import-hardening-m1-probe"],
    });
  ...
  it("has one generated permission file for the exact command", () => {
    const directory = new URL("../../src-tauri/permissions/autogenerated/", import.meta.url);
    const files = readdirSync(directory).sort();
    expect(files).toEqual(["run_import_hardening_m1_probe.toml"]);
    const permission = readFileSync(new URL(files[0], directory), "utf8");
    expect(permission.match(/^identifier = /gm)).toHaveLength(2);
    expect(permission).toContain('identifier = "allow-run-import-hardening-m1-probe"');
    expect(permission).toContain('commands.allow = ["run_import_hardening_m1_probe"]');
    expect(permission).toContain('identifier = "deny-run-import-hardening-m1-probe"');
    expect(permission).toContain('commands.deny = ["run_import_hardening_m1_probe"]');
    expect(permission).not.toMatch(/commands\.(?:allow|deny) = \[[^\]]*,/);
  });

  it("aligns the only capability and development ports", () => {
    ...
    expect(config.build?.devUrl).toBe("http://localhost:1423");
