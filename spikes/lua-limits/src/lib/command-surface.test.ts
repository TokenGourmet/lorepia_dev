...
describe("frontend native command surface", () => {
  it("contains one direct literal invoke and only the no-argument M-1 probe", () => {
    ...
    expect(coreImports).toEqual([
      { source: "lua-probe.ts", imported: "invoke", local: "invoke" },
    ]);
    expect(unsafeInvokeBindingUses).toEqual([]);
    expect(invocations).toEqual([
      {
        callee: "invoke",
        command: "run_lua_limits_m1_probe",
        arguments: 1,
      },
    ]);
  });
  ...
  it("imports invoke in only the bounded protocol module", () => {
    ...
    expect(importers).toEqual(["lua-probe.ts"]);
  });
...
describe("native capability surface", () => {
  it("grants the exact probe command only to the main WebView", () => {
    ...
    expect(capability).toEqual({
      ...
      identifier: "default",
      description: "Exact Lua-limits M-1 probe capability for the trusted main WebView",
      webviews: ["main"],
      permissions: ["allow-run-lua-limits-m1-probe"],
    });
  ...
  it("has one generated permission file for the exact command", () => {
    ...
    expect(files).toEqual(["run_lua_limits_m1_probe.toml"]);
    const permission = readFileSync(new URL(files[0], directory), "utf8");
    expect(permission.match(/^identifier = /gm)).toHaveLength(2);
    expect(permission).toContain('identifier = "allow-run-lua-limits-m1-probe"');
    expect(permission).toContain('commands.allow = ["run_lua_limits_m1_probe"]');
    expect(permission).toContain('identifier = "deny-run-lua-limits-m1-probe"');
    expect(permission).toContain('commands.deny = ["run_lua_limits_m1_probe"]');
